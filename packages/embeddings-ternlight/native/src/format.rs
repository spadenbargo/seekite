...
pub enum FormatError {
    ...
    EmbeddingFormatMismatch {
        expected: u8,
        got: u8,
    },
    UnsupportedWeightsFormat(u8),
    Sha256Mismatch,
    /// The layout walk did not consume the body exactly — the packer and the
    /// engine disagree about the wire format, or the buffer is truncated.
    LengthMismatch {
        expected: usize,
        actual: usize,
    },
...
/// Human-readable rendering of a format label, used in error messages.
pub fn embedding_format_name(tag: u8) -> &'static str {
    match tag {
        EMB_FP32 => "fp32",
        EMB_INT8 => "int8",
        EMB_TERNARY => "ternary",
...
impl core::fmt::Display for FormatError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            FormatError::TooShort => write!(
                f,
                "model buffer is too short to contain a header + sha256 trailer"
            ),
            FormatError::BadMagic => write!(f, "bad magic — expected a packed `TERN` model"),
            FormatError::UnsupportedVersion(v) => {
                write!(
                    f,
                    "unsupported wire-format version {v} (engine speaks v{FORMAT_VERSION})"
                )
            }
            FormatError::UnknownEmbeddingFormat(t) => {
                write!(f, "unknown embedding_format byte {t}")
            }
            FormatError::EmbeddingFormatMismatch { expected, got } => write!(
                f,
                "embedding-format mismatch: this build handles {} (id={}), the model is packed as {} (id={})",
                embedding_format_name(*expected),
                expected,
                embedding_format_name(*got),
                got,
            ),
            FormatError::UnsupportedWeightsFormat(t) => {
                write!(f, "unsupported weights_format byte {t}")
            }
            FormatError::Sha256Mismatch => {
                write!(f, "sha256 mismatch — the model bytes are corrupt")
            }
            FormatError::LengthMismatch { expected, actual } => write!(
                f,
                "layout walk consumed {expected} bytes but the model body is {actual} bytes                  (wire-format drift between packer and engine, or a truncated buffer)"
            ),
        }
    }
}
