...
/// Embed against the process-global model. Panics if no model is installed —
/// see [`crate::model::get`].
pub fn embed(text: &str) -> Vec<f32> {
    embed_with(model::get(), text)
}

/// Embed against an explicitly held model.
///
/// This is the instance-based entry point: it borrows the model rather than
/// reaching for the global `OnceLock`, so a host can keep several models alive
/// at once (`mini` + `base`) and can run this from any thread. The function is
/// pure — no shared mutable state, all scratch buffers are call-local — so
/// concurrent calls on the same `&LoadedModel` are safe and, more importantly,
/// each produce exactly the same bytes a sequential call would.
pub fn embed_with(m: &LoadedModel, text: &str) -> Vec<f32> {
    ...
    parametric_layer_norm(
        &x,
        &m.weights.ln_final_w,
        &m.weights.ln_final_b,
        n_active,
        d_model,
        &mut x_final,
    );
    ...
    for value in pooled.iter_mut().take(d_model) {
        *value *= inv_n;
    }
    ...
    fp32_linear(
        &pooled,
        &m.weights.projection_w,
        &m.weights.projection_b,
        d_model,
        output_dim,
        &mut projected,
    );
...
// Q/K/V geometry and the two caller-owned scratch buffers are intentionally
// explicit at this internal numerical boundary.
